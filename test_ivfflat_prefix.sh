...
CONFIG="cfg/ivfflat-1m.json"
CSV_PREFIX="data/ivfflat/data"
EXTRA_CSV="data/ivfflat/extra_data.csv"
...
if [ "$SKIP_CREATE" = true ]; then
    ...
    if ! ls "${CSV_PREFIX}"*.csv > /dev/null 2>&1 || [ ! -f "$EXTRA_CSV" ]; then
        echo "Error: --skip-create specified, but required data files are missing."
        echo "Please ensure files matching '${CSV_PREFIX}*.csv' and '$EXTRA_CSV' exist or run without --skip-create first."
        exit 1
    fi
    echo "Data files found."
else
    echo "=== Step 1: Data Generation ==="
    python3 gen.py -f $CONFIG --prefix $CSV_PREFIX -s 1234 -p 2
    python3 gen.py -f $CONFIG -o $EXTRA_CSV -s 5678 --start-id 1000001 -n 10000

    echo "=== Step 2: Setup Table and IVFflat Index ==="
    python3 create.py -f $CONFIG --prefix $CSV_PREFIX
fi

echo "=== Step 3: Recall Tests ==="
python3 recall.py -f $CONFIG -m normal -n 1000 -t 8 --prefix $CSV_PREFIX
echo "--- Pre Mode ---"
python3 recall.py -f $CONFIG -m pre -n 1000 -t 8 --i32v 50 --prefix $CSV_PREFIX
echo "--- Post-filtering Mode ---"
python3 recall.py -f $CONFIG -m post -n 1000 -t 8 --i32v 50 --prefix $CSV_PREFIX
echo "--- Force Mode ---"
python3 recall.py -f $CONFIG -m force -n 1000 -t 8 --i32v 50 --prefix $CSV_PREFIX
echo "--- CSV Input Mode ---"
python3 recall.py -f $CONFIG -m normal -n 1000 -t 8 --prefix $CSV_PREFIX --start-id 0
